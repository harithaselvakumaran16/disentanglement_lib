...
class VaeTest(parameterized.TestCase, torch_testing.TestCase):
  ...
  def test_compute_gaussian_kl(self, mean, logvar, target_low, target_high):
    mean_tf = torch.tensor(mean, dtype=np.float32)
    logvar_tf = torch.tensor(logvar, dtype=np.float32)
  ...
  def test_regularize_diag_off_diag_dip(self, matrix, target_low, target_high):
    matrix_tf = torch.tensor(matrix, dtype=np.float32)
    with self.test_session() as sess:
  ...
  def test_total_correlation(self, num_dim, target_low, target_high):
    ...
    z = torch.normal(shape=(10000, num_dim))
    z_mean = torch.zeros(shape=(10000, num_dim))
    z_logvar = torch.zeros(shape=(10000, num_dim))
    with self.test_session() as sess:
